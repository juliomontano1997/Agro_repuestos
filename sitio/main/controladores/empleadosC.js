angular.module('moduloAdministrador')
    .controller('CtrlEmpleados', function ($scope, $location,$http,Conexion)
    {
        $scope.empleados;
        $scope.telefonos;
        $scope.correos;
        ...
        {
            Conexion.getDatos(function (datos) { console.log(datos); $scope.empleados = datos;},"get_personas_tipo", '?tipo=E');
        };

        $scope.cargarEmpleado= function(indice)
        {
            console.log($scope.empleados);
            $scope.cedula_edicion = $scope.empleados[indice].r_cedula;
            document.getElementById("e_cedula").value = $scope.empleados[indice].r_cedula;
            document.getElementById("e_nombre").value =$scope.empleados[indice].r_nombre;
            document.getElementById("e_apellido1").value =  $scope.empleados[indice].r_apellido1;
            document.getElementById("e_apellido2").value =  $scope.empleados[indice].r_apellido2;
            if( $scope.empleados[indice].r_genero===true)
            {
            ...
            $scope.cargarCorreos($scope.empleados[indice].r_cedula);
            $scope.cargarTelefonos($scope.empleados[indice].r_cedula);
            $scope.cargarDirecciones($scope.empleados[indice].r_cedula);
            $scope.cargarTodosDistritos();
            $('#edicionEmpleado').modal('show');
        };
        ...
        $scope.nuevoEmpleado= function ()
        {
            ...
            var gen=document.getElementsByName("gender")[0].checked;
            var datos2 = "?cedula="+datos[0].value+"&nombre="+datos[1].value+"&apellido1="+datos[2].value+"&apellido2="+datos[3].value+"&genero="+gen+"&tipo=E";
